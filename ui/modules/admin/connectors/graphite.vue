    ...
    <v-form>
        ...
        <v-form-block>
            <label>{{ $t("label.connectors.url") }} <v-icon icon="question-circle"
                v-tooltip.right="$t('help.providers.url', ['Graphite'])"></v-icon></label>
        ...
        <v-form-block>
            <label>{{ $t("label.connectors.match_pattern") }}</label>
            <v-input type="text" v-model="settings.pattern"></v-input>
        </v-form-block>
